...
def check():
    ...
    if not empty(b1) or not empty(b2)or not empty(b2)or not empty(b3)or not empty(b4)or not empty(b5)or not empty(b6)or not empty(b7)or not empty(b8)or not empty(b9):
        if b1 == b2 and b2 == b3 :
            win(btn1["text"])
        if b4 == b5 and b5 == b6 :
            win(btn4["text"])
